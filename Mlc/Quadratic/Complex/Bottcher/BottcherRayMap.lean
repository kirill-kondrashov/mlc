...
Idea: if `proxy_bottcher_map c` is a continuous, injective, open map onto an open set,
...
lemma ray_map_eq_on_symm
    (c : ℂ)
    (h_emb : IsOpenEmbedding (proxy_bottcher_map c))
    (h_surj : {w : ℂ | 1 < ‖w‖} ⊆ Set.range (proxy_bottcher_map c)) :
    Set.EqOn (external_ray_map c)
      (h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).symm {w : ℂ | 1 < ‖w‖} := by
  intro w hw
  have hw' : w ∈ Set.range (proxy_bottcher_map c) := h_surj hw
  rcases hw' with ⟨a, rfl⟩
  have h_inv :
      proxy_bottcher_map c (external_ray_map c (proxy_bottcher_map c a)) =
        proxy_bottcher_map c a := by
    have hw' : 1 < ‖proxy_bottcher_map c a‖ := by
      simpa using hw
    simpa using external_ray_map_right_inverse c (proxy_bottcher_map c a) hw'
  have h_symm :
      proxy_bottcher_map c
        ((h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).symm (proxy_bottcher_map c a)) =
          proxy_bottcher_map c a := by
    simpa using
      (Topology.IsOpenEmbedding.toOpenPartialHomeomorph_right_inv (f := proxy_bottcher_map c)
        (h := h_emb) (x := proxy_bottcher_map c a) ⟨a, rfl⟩)
  have h_eq :
      external_ray_map c (proxy_bottcher_map c a) =
        (h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).symm (proxy_bottcher_map c a) := by
    have hpre :
        proxy_bottcher_map c (external_ray_map c (proxy_bottcher_map c a)) =
          proxy_bottcher_map c
            ((h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).symm (proxy_bottcher_map c a)) := by
      calc
        proxy_bottcher_map c (external_ray_map c (proxy_bottcher_map c a)) = proxy_bottcher_map c a := h_inv
        _ = proxy_bottcher_map c
              ((h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).symm (proxy_bottcher_map c a)) := by
...
lemma ray_map_target_subset
    ...
    (h_surj : {w : ℂ | 1 < ‖w‖} ⊆ Set.range (proxy_bottcher_map c)) :
    {w : ℂ | 1 < ‖w‖} ⊆
      (h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).target := by
  intro w hw
  have hw' : w ∈ Set.range (proxy_bottcher_map c) := h_surj hw
  simpa using
    (show w ∈ (h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)).target from
      by simpa [Topology.IsOpenEmbedding.toOpenPartialHomeomorph_target] using hw')

theorem ray_map_continuous_on
    (c : ℂ)
    (h_cont : Continuous (proxy_bottcher_map c))
    (h_inj : Function.Injective (proxy_bottcher_map c))
    (h_open : IsOpenMap (proxy_bottcher_map c))
    (h_surj : {w : ℂ | 1 < ‖w‖} ⊆ Set.range (proxy_bottcher_map c)) :
    ContinuousOn (external_ray_map c) {w | 1 < ‖w‖} := by
  -- 1) Promote `proxy_bottcher_map c` to an open embedding.
  have h_emb : IsOpenEmbedding (proxy_bottcher_map c) :=
  ...
  let e := h_emb.toOpenPartialHomeomorph (proxy_bottcher_map c)
...
theorem external_ray_map_continuousOn_exterior_of_inj_open
    ...
    (h_open : IsOpenMap (proxy_bottcher_map c)) :
    ContinuousOn (external_ray_map c) {w | 1 < ‖w‖} := by
  have h_surj : {w : ℂ | 1 < ‖w‖} ⊆ Set.range (proxy_bottcher_map c) := by
    intro w hw
    rcases (proxy_bottcher_map_surj c w hw) with ⟨a, _ha, rfl⟩
    exact ⟨a, rfl⟩
