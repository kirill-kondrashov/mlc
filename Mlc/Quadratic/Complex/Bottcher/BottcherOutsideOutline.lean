...
/-!
Outline: proving `proxy_bottcher_map_inj_on_outside`.

This file records the minimal analytic/dynamical lemmas needed to replace the
axiom `proxy_bottcher_map_inj_on_outside` with a proof. No axioms are introduced here.

Roadmap (target: injectivity on `outside_disk c`):

1. Analyticity near infinity.
   - Show the current proxy `proxy_bottcher_map` is holomorphic on a neighborhood of the exterior.
   - Suggested lemma:
     `proxy_bottcher_map_analytic_on_outside :
        AnalyticOnNhd ℂ (Quadratic.proxy_bottcher_map c) (outside_disk c)`
...
3. Nonvanishing derivative on the exterior.
   - Use the conjugacy `φ(f(z)) = φ(z)^2` and the asymptotics to show:
     `deriv (Quadratic.proxy_bottcher_map c) z ≠ 0` for `z ∈ outside_disk c`.
...
4. Properness / degree-one argument.
   - Show the current proxy `proxy_bottcher_map` is proper on the exterior and has degree 1.
   - Combined with local injectivity, this yields global injectivity:
     `Set.InjOn (Quadratic.proxy_bottcher_map c) (outside_disk c)`.

5. Range characterization (optional, but standard):
   - `‖proxy_bottcher_map c z‖ > 1` for `z` in the exterior, and surjectivity onto
      `{w | 1 < ‖w‖}`. This also yields the preimage inclusion axiom.

Once the above lemmas are formalized, the axioms
`proxy_bottcher_map_inj_on_outside` and `proxy_bottcher_map_preimage_exterior_subset_outside`
can be removed, and `bottcher_theorem_outside` becomes a direct corollary.
-/

lemma proxy_bottcher_map_norm_gt_one_of_outside (c : ℂ) {z : ℂ} (hz : z ∈ outside_disk c) :
  ...
  exact proxy_bottcher_map_norm_gt_one_of_basin c z hz_basin hpos

def bottcher_preimage_exterior_outside_cond (c : ℂ) : Prop :=
  ∀ z, 1 < ‖Quadratic.proxy_bottcher_map c z‖ → z ∈ outside_disk c

lemma proxy_bottcher_map_preimage_exterior_subset_outside_of
    (c : ℂ) (hcond : bottcher_preimage_exterior_outside_cond c) :
    (Quadratic.proxy_bottcher_map c) ⁻¹' {w : ℂ | 1 < ‖w‖} ⊆ outside_disk c := by
  intro z hz
  have : 1 < ‖Quadratic.proxy_bottcher_map c z‖ := by
...
lemma bottcher_preimage_exterior_outside_cond_of_basin
  ...
  intro z hz
  exact hbasin z (proxy_bottcher_map_norm_gt_one_implies_basin c hz)

lemma proxy_bottcher_map_analytic_on_outside_of_slit (c : ℂ)
    (hslit : {z : ℂ | ‖z‖ > ‖c‖ + 2} ⊆ slit_orbit c) :
    AnalyticOnNhd ℂ (Quadratic.proxy_bottcher_map c) {z : ℂ | ‖z‖ > ‖c‖ + 2} := by
  ...
  have hUbasin : {z : ℂ | ‖z‖ > ‖c‖ + 2} ⊆ Quadratic.basin_of_infinity c := by
    ...
    simpa [outside_disk, Quadratic.basin_of_infinity] using hz'
  exact proxy_bottcher_map_analyticOnNhd_open c _ hUopen hslit hUbasin

def atInfinity : Filter ℂ :=
  Filter.comap (fun z : ℂ => ‖z‖) atTop

def bottcher_normalized_at_infty (c : ℂ) : Prop :=
  Tendsto (fun z => (Quadratic.proxy_bottcher_map c z) / z) atInfinity (𝓝 (1 : ℂ))
...
def bottcher_normalized_at_infty_norm (c : ℂ) : Prop :=
  Tendsto (fun z => ‖Quadratic.proxy_bottcher_map c z‖ / ‖z‖) atInfinity (𝓝 (1 : ℝ))

def bottcher_deriv_nonzero_on_outside (c : ℂ) : Prop :=
  ∀ z, z ∈ outside_disk c → deriv (Quadratic.proxy_bottcher_map c) z ≠ 0
