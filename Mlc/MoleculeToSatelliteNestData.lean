...
/-- Canonical-depth variant: produce a uniform conformal lower bound along the
    tower-selected principal annuli. The root-facing residual target is direct:
    its conclusion does not depend on a Molecule witness. -/
...
theorem moleculeBridgeTarget_of_moleculeUniformBridgeTarget
  ...
  rcases hTarget c hc hTower with ⟨μ, hμ_pos, hμ_lb⟩
  refine ⟨{
...
theorem lc_of_moleculeUniformBridgeTarget
    ...
    exact lc_at_of_shrink_of_connected_at c hc
      (para_puzzle_piece_inter_mandelbrot_connected_proved c hc)
      (parameter_shrink_of_moleculeUniformBridgeTarget hTarget c hc hTower)
...
theorem bridge_of_moleculeUniformBridgeTarget
    ...
    ∀ (c : ℂ) (hc : c ∈ MLC.Quadratic.MandelbrotSet) (_h : SatelliteRenormalizableTower c),
      MLC.LocallyConnectedAt MLC.Quadratic.MandelbrotSet ⟨c, hc⟩ := by
  intro h_mol c hc hTower
  exact lc_of_moleculeUniformBridgeTarget hTarget c hc hTower
