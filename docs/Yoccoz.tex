...
\begin{itemize}
    ...
    \item \textbf{Case 2: $c \notin \mathcal{M}$}:
        \begin{itemize}
            ...
            \item If $c \notin \mathcal{M}$, then the puzzle pieces eventually become empty (\texttt{dynamical\_puzzle\_piece\_empty\_of\_large\_n}).
            \item This means the annuli become empty for large $n$, so their moduli are 0.
            \item A sequence of non-negative terms that is eventually 0 has a finite sum (\texttt{summable\_of\_finite\_support}).
